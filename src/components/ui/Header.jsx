...
const Header = () => {
  ...
  const navigationItems = [
    ...
    { path: '/stories-games', label: 'Stories', labelTelugu: 'కథలు', icon: 'BookOpen' },
  ];
            ...
            {isMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-popover border border-border rounded-tribal shadow-tribal-md z-50">
                <div className="py-2">
                  ...
                  {isAuthenticated && (
                    <button
                      onClick={() => { logout(); navigate('/login-authentication'); setIsMenuOpen(false); }}
                      className="w-full flex items-center px-4 py-2 text-sm font-medium text-left smooth-transition hover:bg-muted text-foreground"
                    >
                      <Icon name="LogOut" size={16} className="mr-3" />
                      <span>{getText('Logout', 'లాగౌట్')}</span>
                    </button>
                  )}
                </div>
              </div>
